...
> **3行で分かる**
> 1. Markdown → モバイル対応HTMLへの変換パイプラインを解説
> 2. convert.py でMD→HTML変換、CIでリンク切れ・ビルドを自動保証
> 3. このガイドサイト自身がSSOTノウハウの公開・成果物版
...
各章のH1直後に「⚡ 3行で分かる」枠を表示し、スマホの隙間時間でも各章の要点を3行で把握できる仕組み。

### 使い方（source MD側）

各章MDの **H1直後** に blockquote で3行サマリーを書く:
...
`convert_tldr(html)` が **H1直後の最初のblockquote** を `<aside class="tldr">` に変換。`'3行で分かる'` を含まなければ変換しない（後方互換）。CSS（`.tldr`）が ⚡アイコン付きの枠を表示し、1行目の「3行で分かる」見出しは `::before` で代替表示して非表示化。

### ⚠️ 呼出順序の注意（重要）

`convert_tldr` は `enhance_html` の **前**に呼ぶこと。後呼びだと `enhance_html` の callout 変換（`<blockquote><p>…</p></blockquote>`）にTLDRが食われ、aside化されない。

```python
# OK（前呼び）
html_body = convert_tldr(html_body)   # 先にTLDRをaside化
html_body = enhance_html(html_body)   # 残りのblockquoteをcallout化

# NG（後呼び・TLDRがcalloutに食われる）
html_body = enhance_html(html_body)
html_body = convert_tldr(html_body)
```

> 本家 claude-code-guide は後呼びで動いているが、callout変換を持つガイドでは前呼びが正しい（2026-07-17 横展開時に発見）。

### 横展開状況（2026-07-17）

| リポジトリ | 方式 | ページ |
|---|---|---|
| claude-code-guide | convert.py（後呼び・本家） | 全章 |
| ssot-guide / dev-textbook / ssh-guide | convert.py（前呼び） | 各全章 |
| loop-engineering-guide | HTML直接記述（aside直埋め） | 4章 |
| python-reading-guide | 既存cheat-section強化（学習型・配置変更） | 30章（interactive HTML） |

> **学習型教科書では配置を変える（2026-07-17 python-reading-guide で発見）**: 上記はすべて**リファレンス型**（調べる目的）を前提に「冒頭 H1 直下の結論ファースト TLDR」を置く。しかし**学習型教科書**（段階的に学ぶ目的・python-reading-guide 等）では冒頭に結論を出すと学習体験を逆転させ、既存の concept-card（冒頭1行概念）とも重複する。学習型では **既存のまとめセクション（末尾の cheat-section 等）を強化する位置**に「3行で分かる」を置くのが正しい。ガイド平易化の前には必ず**既存のまとめ要素の有無**を確認し、既存があれば新規 aside ではなく強化で重複を避けること。
...
| `build-deploy.yml` の `cache: 'pip'` | requirements.txt / pyproject.toml 未配置リポだと `setup-python` がエラーになる。該当行を削除（dev-textbook 実例） |
...
- **dev-textbook**: https://github.com/fukukei23/dev-textbook （ssot-guide 流用で新規立ち上げ・読む系14章の統合教科書。sed一括置換でブランド名/URL書き換え・build-deploy.yml の `cache:'pip'` 落とし穴実例）
